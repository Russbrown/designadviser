...
CREATE POLICY IF NOT EXISTS "Public read access for design images" ON storage.objects
  FOR SELECT USING (bucket_id = 'design-images');

-- Allow authenticated users to upload files
CREATE POLICY IF NOT EXISTS "Authenticated users can upload design images" ON storage.objects
...
CREATE POLICY IF NOT EXISTS "Users can update their own design images" ON storage.objects
  FOR UPDATE USING (
    bucket_id = 'design-images' AND 
...
CREATE POLICY IF NOT EXISTS "Users can delete their own design images" ON storage.objects
  FOR DELETE USING (
    bucket_id = 'design-images' AND 
    auth.uid()::text = (storage.foldername(name))[1]
  );